...
/**
 * Converts a {@link SourceDocument} into a Lucene {@link Document}, ready to be
 * indexed.
 *
 * @param <T> type of the source document
 */
public class EnhancedLuceneDocumentGenerator<T extends SourceDocument> implements LuceneDocumentGenerator<T> {
  protected IndexArgs args;

  protected EnhancedLuceneDocumentGenerator() {
  }
   ...
   */
  public EnhancedLuceneDocumentGenerator(IndexArgs args) {
    this.args = args;
  }
  ...
  public Document createDocument(T src) throws GeneratorException {
    ...
    try {
      ...
      // Catch and rethrow; indexer will eat the exception at top level and increment
    ...
    if (src instanceof MultifieldSourceDocument) {
      ((MultifieldSourceDocument) src).fields().forEach((k, v) -> {
        if (k == IndexArgs.ENTITY) {
          ...
          // Only index fields that have been explicitly referenced in -fields parameter
          // of indexing program.
