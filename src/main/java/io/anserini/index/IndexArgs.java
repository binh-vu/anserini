...
public class IndexArgs {
    ...
    // This is the name of the field in the Lucene document where the docid is
    // stored.
    public static final String ID = "id";

    // This is the name of the field in the Lucene document that should be searched
    // by default.
    public static final String CONTENTS = "contents";

    // This is the name of the field in the Lucene document where the raw document
    // is stored.
    public static final String RAW = "raw";

    // This is the name of the field in the Lucene document where the entity
    // document is stored.
    ...
    @Option(name = "-threads", metaVar = "[num]", required = true, usage = "Number of indexing threads.")
    public int threads;

    @Option(name = "-collection", metaVar = "[class]", required = true, usage = "Collection class in package 'io.anserini.collection'.")
    ...
    @Option(name = "-verbose", forbids = {
            "-quiet" }, usage = "Enables verbose logging for each indexing thread; can be noisy if collection has many small file segments.")
    ...
    @Option(name = "-optimize", usage = "Boolean switch to optimize index (i.e., force merge) into a single segment; costly for large collections.")
    ...
    @Option(name = "-uniqueDocid", usage = "Removes duplicate documents with the same docid during indexing. This significantly slows indexing throughput "
            +
    ...
    @Option(name = "-solr", forbids = { "-index", "-es" }, usage = "Indexes into Solr.")
    public boolean solr = false;

    @Option(name = "-solr.batch", metaVar = "[n]", usage = "Solr indexing batch size.")
    public int solrBatch = 1000;

    @Option(name = "-solr.commitWithin", metaVar = "[s]", usage = "Solr commitWithin setting (in seconds).")
    public int solrCommitWithin = 60;

    @Option(name = "-solr.index", metaVar = "[name]", usage = "Solr index name.")
    public String solrIndex = null;

    @Option(name = "-solr.zkUrl", metaVar = "[urls]", usage = "Solr ZooKeeper URLs (comma separated list).")
    public String zkUrl = null;

    @Option(name = "-solr.zkChroot", metaVar = "[path]", usage = "Solr ZooKeeper chroot")
    public String zkChroot = "/";

    @Option(name = "-solr.poolSize", metaVar = "[n]", usage = "Solr client pool size.")
    public int solrPoolSize = 16;

    // Elasticsearch options

    @Option(name = "-es", forbids = { "-index", "-solr" }, usage = "Indexes into Elasticsearch.")
    public boolean es = false;

    @Option(name = "-es.index", metaVar = "[name]", usage = "Elasticsearch index name.")
    public String esIndex = null;

    @Option(name = "-es.batch", metaVar = "[n]", usage = "Elasticsearch batch index requests size.")
    public int esBatch = 1000;

    @Option(name = "-es.bulk", metaVar = "[n]", usage = "Elasticsearch max bulk requests size in bytes.")
    public int esBulk = 80000000;

    @Option(name = "-es.hostname", metaVar = "[host]", usage = "Elasticsearch host.")
    public String esHostname = "localhost";

    @Option(name = "-es.port", metaVar = "[port]", usage = "Elasticsearch port number.")
    public int esPort = 9200;

    @Option(name = "-es.user", metaVar = "[username]", usage = "Elasticsearch user name.")
    public String esUser = "elastic";

    @Option(name = "-es.password", metaVar = "[password]", usage = "Elasticsearch password.")
    public String esPassword = "changeme";

    @Option(name = "-es.poolSize", metaVar = "[num]", usage = "Elasticsearch client pool size.")
    public int esPoolSize = 10;

    @Option(name = "-es.connectTimeout", metaVar = "[ms]", usage = "Elasticsearch (low level) REST client connect timeout (in ms).")
    public int esConnectTimeout = TIMEOUT;

    @Option(name = "-es.socketTimeout", metaVar = "[ms]", usage = "Elasticsearch (low level) REST client socket timeout (in ms).")
    public int esSocketTimeout = TIMEOUT;

    // Sharding options

    @Option(name = "-shard.count", metaVar = "[n]", usage = "Number of shards to partition the document collection into.")
    public int shardCount = -1;

    @Option(name = "-shard.current", metaVar = "[n]", usage = "The current shard number to generate (indexed from 0).")
